...
class Telnet3270Client extends EventEmitter {
  constructor() {
    ...
    this.terminalType = 'IBM-3279-2-E';
    this.deviceName = '';
    this.pending = [];
    this.tn3270e = false;
  ...
  write(buffer) {
    const escaped = escapeIac(buffer);
    const payload = this.tn3270e ? addTn3270EDataHeader(escaped, this.nextTn3270ESequence()) : escaped;
    const framed = appendEor(payload);
  ...
  handleTelnet(chunk) {
    const data = this.pending.length > 0 ? Buffer.concat([Buffer.from(this.pending), chunk]) : chunk;
    this.pending = [];
    const out = [];
    const frames = [];
    for (let i = 0; i < data.length; i += 1) {
      ...
      if (command === IAC) {
        ...
        if (i + 1 >= data.length) {
          this.pending = [IAC, SB, ...data.subarray(start)];
          break;
        }
        this.handleSubnegotiation(data.subarray(start, i));
        i += 1;
      }
    }
