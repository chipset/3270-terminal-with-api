...
const { terminalSize } = require('./deviceTypes');

class EmulatorSession extends EventEmitter {
  constructor(options = {}) {
    super();
    const initialSize = terminalSize(options.terminalType || 'IBM-3279-2-E');
    this.options = {
  ...
  async connect(options = {}) {
    ...
    this._reconnectTimer = null;
    const terminalType = options.terminalType ?? this.options.terminalType;
  ...
  setTerminalType(terminalType) {
    const size = terminalSize(terminalType);
    this.options.terminalType = terminalType || this.options.terminalType;
    if (this.screen.rows !== size.rows || this.screen.cols !== size.cols) {
      this.screen = new ScreenBuffer(size.rows, size.cols);
      this.emit('trace', `screen resized for ${this.options.terminalType}: ${size.rows}x${size.cols}`);
    }
  }
