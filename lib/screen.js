...
class ScreenBuffer {
  ...
  putChar(ch, options = {}) {
    ...
    this.applyCharacterCell(cell, ch, field, Boolean(options.fromHost && this.currentGraphic.explicit));
    if (!options.fromHost) {
  ...
  applyCharacterCell(cell, ch, field, useStreamGraphic = false) {
    const graphic = useStreamGraphic ? this.currentGraphic : (field ? field.graphic : this.currentGraphic);
    cell.char = ch[0] ?? ' ';
    ...
    cell.explicitGraphic = useStreamGraphic;
    cell.explicitColor = Boolean(graphic.explicitColor);
  }
  ...
  startField(attributeByte, extendedAttributes = {}) {
    ...
    Object.assign(cell, {
      ...
      explicitColor: Boolean(field.graphic.explicitColor)
    });
    this.setAddress(address + 1);
    this.syncStreamGraphicFromField(field);
  ...
  modifyCurrentField(attributeByte) {
    ...
    field.attributeByte = attributeByte;
    field.graphic = decodeExtendedAttributes(field.extendedAttributes, decodeFieldAttribute(attributeByte));
    this.syncStreamGraphicFromField(field);
    this.applyFieldMetadata();
  }
  ...
  modifyCurrentFieldAttributes(attributes = {}) {
    ...
    field.graphic = decodeExtendedAttributes(field.extendedAttributes, decodeFieldAttribute(field.attributeByte));
    this.syncStreamGraphicFromField(field);
    this.applyFieldMetadata();
  }
  ...
  // SF/SFE begins a new field; SA applies until the next field. Clear explicit SA so later fields use their own colors.
  syncStreamGraphicFromField(field) {
    this.currentGraphic = { ...field.graphic, explicit: false };
  }
  ...
  repeatToAddress(address, char) {
    ...
    do {
      const field = this.getFieldForAddress(current);
      this.applyCharacterCell(this.cells[current], char, field, this.currentGraphic.explicit);
  ...
  getFieldText(field) {
    ...
    if (start <= end) {
      ...
      // Field wraps around the end of the screen buffer to the beginning
      for (let address = start; address < this.size; address += 1) {
        chars.push(this.cells[address].char);
      }
      for (let address = 0; address < end; address += 1) {
        chars.push(this.cells[address].char);
      }
    }
...
function decodeExtendedAttributes(attributes = {}, base = {}) {
  const explicitColor = attributes[0x42] !== undefined || Boolean(base.explicitColor);
  const color = decodeColor(attributes[0x42]) ?? base.color ?? baseColorFromFieldAttribute(base);
...
function baseColorFromFieldAttribute(base = {}) {
  if (base.hidden) {
    return 'green';
  }
  if (base.protected) {
    return base.intensified ? 'white' : 'blue';
  }
  return base.intensified ? 'red' : 'green';
}
