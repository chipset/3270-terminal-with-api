...
function buildQueryReply() {
  return Buffer.concat([
    ...
    Buffer.from([
      0x00, 0x18, 0x81, 0x86, 0x00, 0x00, 0x0e, 0x00,
...
function stripTn3270EHeader(data) {
  if (isLikelyTn3270EHeader(data)) {
    return data.subarray(5);
  }
  return data;
}

function isLikelyTn3270EHeader(data) {
  if (data.length <= 5) {
    return false;
  }
  // RFC 2355 data header: data-type, request-flag, response-flag, seq-number(2).
  // Hosts commonly send non-zero flags and sequence numbers, so do not require zeroes.
  return data[0] <= 0x07 && data[1] <= 0x07 && data[2] <= 0x0f && looksLike3270DataStream(data.subarray(5));
}
