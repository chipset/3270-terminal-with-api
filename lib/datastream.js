...
function applyDataStream(buffer, screen) {
  ...
  for (let i = start; i < data.length; i += 1) {
    ...
    if (byte === ORDERS.sba) {
      ...
      recordColorAttribute(colorAttributes, attributes[0x42]);
      ...
      if (data[i + 1] === 0x42) {
        recordColorAttribute(colorAttributes, data[i + 2]);
      }
  ...
  return {
    ...
    orders,
    mode: '3270',
    colorAttributes: summarizeColorAttributes(colorAttributes)
  };
}

function recordColorAttribute(colorAttributes, value) {
  if (value !== undefined) {
    colorAttributes.push(Number(value) & 0xff);
  }
}

function summarizeColorAttributes(colorAttributes) {
  if (colorAttributes.length === 0) {
    return undefined;
  }
  const counts = new Map();
  for (const value of colorAttributes) {
...
function buildQueryReply() {
  return Buffer.concat([
    ...
    // Color Query Reply: extended palette f0–fe (omit ff so telnet IAC escaping does not corrupt the reply).
    // Nc = 0x0f (15) matches the 15 color entries f0–fe that follow the default-color byte.
    Buffer.from([
      0x00, 0x19, 0x81, 0x86, 0x00, 0x00, 0x0f, 0x00,
...
function looksLike3270DataStream(data) {
  // Only check COMMANDS for the first byte. ORDERS (e.g. 0x08 GE == ASCII backspace)
  // appear in the payload, not at position 0, and would produce false positives.
  if (Object.values(COMMANDS).includes(data[0])) {
    return true;
  }
...
function stripTn3270EHeader(data) {
  // Bytes 0–2 are data-type, request-flag, response-flag (all 0x00 for normal data).
  // Bytes 3–4 are the sequence number and must NOT be tested — they increment past 0xff.
  if (data.length > 5 && data[0] <= 0x07 && data[1] === 0x00 && data[2] === 0x00) {
